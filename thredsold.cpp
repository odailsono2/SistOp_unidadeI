...
//*/
    ...
    for (int i = 0 ; i < Nthreads; ++i){
        ...
        // std::cout<<"Matriz:"<<std::endl;auto
        // printMatrix(Prod);
        

        threads[i].th = std::thread (Produto,M1,M2, inicio,qtdElem, std::ref(threads[i].Prod));
...
//---------para terminar
    for (int i = 0 ; i < Nthreads ; ++i){
        ...
        //if(threads[i].th.joinable()){
            ...
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(threads[i].t2 - threads[i].t1);
            
            // Converte a duração para um valor numérico (microsegundos)
            
            long long microseconds = duration.count();

            salvaArq(threads[i].Prod,microseconds, i );
